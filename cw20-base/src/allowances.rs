use cosmwasm_std::{
    attr, Addr, Binary, BlockInfo, Deps, DepsMut, Env,
...
use crate::state::{
    ALLOWANCES, ALLOWANCES_SPENDER, BALANCES, TOKEN_INFO,
};
pub fn execute_increase_allowance(
    ...
    let update_fn =
        |allow: Option<AllowanceResponse>| -> Result<_, _> {
...
pub fn execute_decrease_allowance(
    ...
    let mut allowance =
        ALLOWANCES.load(deps.storage, key)?;
    if amount < allowance.allowance {
        ...
        ALLOWANCES_SPENDER
            .remove(deps.storage, reverse(key));
    }
...
pub fn deduct_allowance(
    ...
    let update_fn =
        |current: Option<AllowanceResponse>| -> _ {
            match current {
                Some(mut a) => {
...
pub fn execute_burn_from(
    ...
    TOKEN_INFO.update(
        deps.storage,
        |mut meta| -> StdResult<_> {
            meta.total_supply =
                meta.total_supply.checked_sub(amount)?;
            Ok(meta)
        },
    )?;
...
mod tests {
    ...
    use cosmwasm_std::{
        coins, CosmosMsg, SubMsg, Timestamp, WasmMsg,
    };
    use cw20::{Cw20Coin, TokenInfoResponse};

    use crate::contract::{
        execute, instantiate, query_balance,
        query_token_info,
    };
    ...
    fn increase_decrease_allowances() {
        ...
        let new_expire = Expiration::AtTime(
            Timestamp::from_seconds(8888888888),
        );
        let msg = ExecuteMsg::IncreaseAllowance {
            spender: spender.clone(),
            amount: raise,
    ...
    fn allowances_independent() {
        ...
        let expires3 = Expiration::AtTime(
            Timestamp::from_seconds(3767626296),
        );
        let msg = ExecuteMsg::IncreaseAllowance {
            spender: spender2.clone(),
            amount: allow3,
    ...
    fn transfer_from_respects_limits() {
        ...
        assert_eq!(
            get_balance(deps.as_ref(), rcpt.clone()),
            transfer
        );
    ...
    fn send_from_respects_limits() {
        ...
        assert_eq!(
            get_balance(deps.as_ref(), contract.clone()),
            transfer
        );
    ...
    fn no_past_expiration() {
        ...
        let expires =
            Expiration::AtHeight(env.block.height);
        let msg = ExecuteMsg::IncreaseAllowance {
            spender: spender.clone(),
            amount: Uint128::new(7777),
            expires: Some(expires),
