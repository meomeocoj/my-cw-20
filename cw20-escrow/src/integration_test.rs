...
use cosmwasm_std::{
    coins, to_binary, Addr, Empty, Uint128,
};
...
use crate::msg::{
    CreateMsg, DetailResponse, ExecuteMsg, InstantiateMsg,
    QueryMsg, ReceiveMsg,
};
...
fn escrow_happy_path_cw20_tokens() {
    ...
    // ensure our balances
    let owner_balance = cash
        .balance::<_, _>(&router.wrap(), owner.clone())
        .unwrap();
    ...
    let details: DetailResponse = router
        .wrap()
        .query_wasm_smart(
            &escrow_addr,
            &QueryMsg::Detail { id: id.clone() },
        )
        .unwrap();
    ...
    let owner_balance = cash
        .balance::<_, _>(&router.wrap(), owner)
        .unwrap();
    assert_eq!(owner_balance, Uint128::new(3800));
    let escrow_balance = cash
        .balance::<_, _>(&router.wrap(), escrow_addr)
        .unwrap();
    assert_eq!(escrow_balance, Uint128::zero());
    let ben_balance =
        cash.balance::<_, _>(&router.wrap(), ben).unwrap();
